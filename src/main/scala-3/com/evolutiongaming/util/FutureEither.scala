...
object FutureEither {
  ...
  def list[L, R](xs: List[FutureEither[L, R]])(implicit executor: ExecutionContext): FutureEither[L, List[R]] = {
    sequence(xs)
  }

  def sequence[L, R, From[A] <: IterableOnce[A], To](
    in: From[FutureEither[L, R]])(implicit
    buildFrom: BuildFrom[From[FutureEither[L, R]], R, To],
    executor: ExecutionContext
  ): FutureEither[L, To] = {

    in.iterator.foldLeft(Future.successful(buildFrom.newBuilder(in)).fe[L]) {
